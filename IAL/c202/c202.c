...
void Stack_Init( Stack *stack ) {
    ...
    // Pokud funkce dostane parametr NULL, provede se funkce Stack_Error, jinak...
    if (stack != NULL){
        // Nastavení vrcholu na -1(prázdný)
        stack->topIndex = -1;     
    }else{
        Stack_Error(SERR_INIT);
        return; // Return přeruší inicializaci.
    }
}
...
int Stack_IsEmpty( const Stack *stack ) {
    ...
    */
    // Pokud je topIndex -1(stack je prázdný), return vrátí 1 => stack je prázdný,
    // else 0, stack není prázdný.
...
int Stack_IsFull( const Stack *stack ) {
    /*
    if (stack->topIndex == FULL-1){
        return 1;
    }else{
        return 0;
    }
    */
    // Pokud je stack plný, return vrátí 1, jinak 0;
    // -1 odečítáme, protože první znak je na indexu 0
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
    ...
    // Pokud je stack prázdný fce Stack_IsEmpty == 1, provede se funkce Stack_Error, jinak...
    if (Stack_IsEmpty(stack) != 1){
        // Do *dataPtr se uloží znak z vrcholu zásobníku, vrchol se určí indexem topIndex.
...
void Stack_Pop( Stack *stack ) {
    ...
    // Pokud není stack prázdný fce Stack_IsEmpty == 0, provede se funkce Stack_Error, jinak...
    if (Stack_IsEmpty(stack) != 1){
        // Ukazatel na vrchol zásobníku se dekrementuje o 1 => odstranění prvku z vrcholu.
        stack->topIndex--;
    }
}
 ...
 * je nekorektní a ošetřete ho voláním procedury Stack_Error(SERRPUSH).
 *
...
void Stack_Push( Stack *stack, char data ) {
    ...
    // Pokud je stack plný, fce Stack_IsFull == 1,provede se funkce Stack_Error, jinak...
    if (Stack_IsFull(stack) != 1){
        // topIndex se inkrementuje(udělá místo pro další znak)
        stack->topIndex++;
        // Následně se do něj zapíše znak z 'data'
